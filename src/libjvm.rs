...
mod dynlib {
    ...
    pub fn libjvm_or_load() -> GlobalResult<&'static Libjvm> {
    ...
    pub fn libjvm_or_load_at(path: &Path) -> GlobalResult<&'static Libjvm> {
        LIBJVM.get_or_try_init(|| load_libjvm_at(path))
    }
}

#[cfg(feature = "dynlib")]
pub use dynlib::{libjvm_or_load, libjvm_or_load_at};
